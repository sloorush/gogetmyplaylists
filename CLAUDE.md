...
- `requirements.txt` — Python deps: spotipy, yt-dlp, yt-dlp-ejs, python-dotenv
...
python3 sync.py                          # full sync (discover + download)
python3 sync.py --discover-only          # just update playlists.json
python3 sync.py --download-only          # just download from existing playlists.json
python3 sync.py --playlist boombap       # single playlist
python3 sync.py --download-only --playlist boombap  # single playlist, skip discovery
...
## Dependencies
- **Python**: spotipy, yt-dlp, yt-dlp-ejs, python-dotenv (installed in `.venv/`)
...
2. `python3 sync.py --download-only --playlist 2hard --dry-run` — shows existing tracks as skipped
3. `python3 sync.py --playlist boombap --max-downloads 3` — downloads a few tracks to verify
4. `ffprobe <file>.mp3` — verify 320kbps bitrate
